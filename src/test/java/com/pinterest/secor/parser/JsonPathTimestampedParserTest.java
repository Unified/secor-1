...
public class JsonPathTimestampedParserTest extends TestCase {
    ...
    public void testExtractPartitions () throws Exception {
        ...
        JsonPathTimestampedParser jsonPathParser = new JsonPathTimestampedParser(mConfig);

        String resultSeconds[] = jsonPathParser.extractPartitions(mMessageWithSecondsTimestamp);
        assertEquals(4, resultSeconds.length);
        assertEquals("gender=female", resultSeconds[0]);
        assertEquals("id=0", resultSeconds[1]);
        assertEquals("dt=2014-07-21",resultSeconds[2]);
        assertEquals("hr=02", resultSeconds[3]);
    }
    ...
    public void testExtractPartitions_nestedJson () throws Exception {
        ...
        String resultSeconds[] = jsonPathParser.extractPartitions(mMessageWithSecondsTimestamp);
        assertEquals(5, resultSeconds.length);
        assertEquals("gender=female", resultSeconds[0]);
        assertEquals("id=0", resultSeconds[1]);
        assertEquals("friends=Rebecca Hardy", resultSeconds[2]);
        assertEquals("dt=2014-07-21", resultSeconds[3]);
        assertEquals("hr=02", resultSeconds[4]);
    }
